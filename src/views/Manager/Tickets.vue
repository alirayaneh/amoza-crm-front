...
const canEdit = (ticket: Ticket): boolean => {
  return (ticket.agents.filter((tic) => tic.id === user.value.id).length ||
    ticket.agents.filter((tic) => tic.pivot.transferred_by === user.value.id).length) &&
    user.value.roles?.filter((role) => role.name === 'SourceMaster').length
    ? true
    : false
}
</script>
...
<template>
  ...
  <header class="mb-3 flex flex-col justify-between gap-3 sm:flex-row-reverse">
    <div class="flex justify-end sm:justify-start">
      <ActionButton :Icon="PlusIcon" text="ایجاد تسک جدید" :action="handleAdd" />
    </div>
        ...
        <tr v-for="ticket in tickets.data" :key="ticket.id">
          <TableTd>
            <component
              :is="
                isAdmin ||
                ticket.agents.filter((agent) => agent.id === user.id).length ||
                ticket.agents.filter((agent) => agent.pivot.transferred_by === user.id).length
                  ? 'router-link'
                  : 'span'
              "
            ...
            <div class="my-2 flex items-center gap-1">
              <div
                class="col h-8 w-8 flex-shrink-0"
                v-for="agent in extractAllUsers(ticket)"
                :key="agent.id"
              >
                <img
            ...
            <TableActions
              v-if="
                ...
                (ticket.status !== 'resolved' &&
                  ...
                  (ticket.agents?.[0]?.id === user.id || !ticket.agents.length || canEdit(ticket)))
              "
            >
              <template v-if="$route.query.trash != 'true'">
                ...
                <MenuItem
                  ...
                  v-if="isAdmin || ticket.agents?.[0]?.id === user.id || canEdit(ticket)"
                >
                  <div
