...
import OrdersModal from '@/components/Manager/Orders/OrdersModal.vue'
import FilterCategories from '@/components/Admin/Categories/FilterCategories.vue'
import FilterOrders from '@/components/Manager/Orders/FilterOrders.vue'
...
import OrderProgress from '@/components/Manager/Orders/OrderProgress.vue'
...
import getOrders from '@/composables/orders/getOrders'
...
const { isAdmin, myDepartments: myroles, isManager } = storeToRefs(authStore)

const hasEditAccess = computed(
  () => (isManager.value && myroles.value?.filter((item) => item === 'Accountants')) || isAdmin
)

useHead({ title: `سفارش ها | ${appTitle}` })
...
const { load: fetchCategories, isLoading: categoriesIsLoading, categories } = getCategories()

const handleAfterSave = async (keepQuery: boolean): Promise<void> => {
  if (!Object.keys(route.query).length) await load()
  else if (!keepQuery) router.push({ query: {} })
  else await load()
}

const { load, orders, isLoading } = getOrders(filters)
...
const handleAdd = () => {
  orderToEdit.value = {} as Order

  newTicketModalIsOpen.value = true
}

const orderToEdit = ref({} as Order)
...
const categoryToDelete = ref({} as Order)
...
const handlePermanentDelete = (category: Order) => {
  categoryToDelete.value = category
...
const categoryToRestore = ref({} as Order)

const handleRestore = (category: Order) => {
...
const handleClosed = () => {
  newTicketModalIsOpen.value = false
  orderToEdit.value = {} as Order
}
const reset = async () => {
  filterPanelIsOpen.value = false
...
<template>
  ...
  <header class="mb-3 flex flex-col justify-between gap-3 sm:flex-row-reverse">
    <div class="flex justify-end sm:justify-start" v-if="hasEditAccess">
      <ActionButton :Icon="PlusIcon" text="ایجاد سفارش" :action="handleAdd" />
    </div>
  ...
  <NotFoundResults
    v-else-if="!isLoading && !orders.data?.length && filtersAreApplied"
    ...
    <template v-slot:default>
      <template v-if="orders.data?.length">
        <tr v-for="order in orders.data" :key="order.id" class="">
          <TableTd>
            {{ order.subject }}
            <OrderProgress :temp-categories="categories" :order-to-edit="order" />
          </TableTd>

          <TableTd>{{ order.total_amount ?? 'وارد نشده' }}</TableTd>

          <TableTd>{{ order.commission_amount ?? 'وارد نشده' }}</TableTd>

          <TableTd>{{ order.my_commissions ?? 'وارد نشده' }}</TableTd>

          <TableTd>{{ order.tasks?.length ?? 'وارد نشده' }}</TableTd>

          <TableTd> {{ order.created_at }}</TableTd>

          <TableTd>
            {{ order.updated_at }}
          </TableTd>

          <TableTd v-if="hasEditAccess">
            <TableActions>
              ...
              <template v-else>
                ...
                <MenuItem class="cursor-pointer">
                  <div
                    @click="handlePermanentDelete(order)"
                    class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                  >
                    Delete permanently
                  </div>
