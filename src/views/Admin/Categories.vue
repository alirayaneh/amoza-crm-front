...
useHead({ title: `وضعیت ها | ${appTitle}` })
...
const selecteddepartments = (catdep: Department[]) => {
  if (!catdep.length) return []
  return departments.value.filter((dep) => {
    return catdep.some((catDep: any) => catDep.department_id == dep.value)
  })
}

const displayDepartments = (catdep: Department[]) => {
  const matchedDepartments = selecteddepartments(catdep)

  if (matchedDepartments.length > 0) {
    return matchedDepartments.map((dep) => dep.name).join(', ')
  } else {
    return 'not selected...'
  }
}
...
<template>
  <CategoryModal
    ...
    :selecteddepartments="
      categoryToEdit?.departments ? selecteddepartments(categoryToEdit.departments) : []
    "
    ...
    <template v-slot:default>
      <template v-if="categories.data?.length">
        <tr v-for="category in categories.data" :key="category.id">
          ...
          <TableTd>
            {{ displayDepartments(category.departments) }}
          </TableTd>
