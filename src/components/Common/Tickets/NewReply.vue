...
const props = defineProps<{ ticketId: number; newRyplayEditorOpen: boolean; content: string }>()
...
<template>
  <Disclosure as="form" v-slot="{ open }">
    ...
    <DisclosurePanel
      class="divide-y rounded-b-lg border shadow"
      :static="props.newRyplayEditorOpen"
    >
      ...
      <div class="flex justify-end bg-gray-50 p-3" dir="ltr">
        <div class="relative z-0 inline-flex rounded-md shadow-sm">
          ...
          <Menu as="span" class="relative -ml-px block">
            ...
            >
              <MenuItems
                class="absolute absolute right-0 bottom-5 mb-5 -mr-1 w-60 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
              >
                <div class="divide-y">
                  <MenuItem
