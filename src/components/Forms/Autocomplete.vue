...
const originalItemNameByValue = (val: Option | undefined): Option => {
  if (val && val.value)
    return filteredItem.value.find((item) => item.value == val?.value) ?? ({} as Option)
  else return {} as Option
}
...
const selected = ref(originalItemNameByValue(props.selected) ?? ({} as Option))
...
watch(
  () => props.selected,
  (value) => {
    if (value && value.value) value = filteredItem.value.find((item) => item.value == value?.value)
